...
class MenuEndless(Menu):
    ...
    def update(self, state: str) -> None:
        ...
        match state:
            case "decrease_cell":
                self.cell_index = self.cell_index - 1 if self.cell_index > 0 else len(self.cell_size_list) - 1
                self.text[0].text = str(self.cell_size_list[self.cell_index])
            case "increase_cell":
                self.cell_index = self.cell_index + 1 if self.cell_index < len(self.cell_size_list) - 1 else 0
                self.text[0].text = str(self.cell_size_list[self.cell_index])
            case "decrease_wave":
                self.wave_count = str(int(self.wave_count) - 1) if int(self.wave_count) > 1 else str(1)
